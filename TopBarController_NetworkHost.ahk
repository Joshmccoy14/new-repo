...
CreateTopBarGUI() {
    ...
    Gui, TopBar:Color, 0x1E1E1E
    Gui, TopBar:Font, s8, Segoe UI
    
    ; 1. Hide Bar
    Gui, TopBar:Add, Button, x5 y3 w55 h24 gToggleBarVisibility, Hide Bar
    ...
    ; 3. Win Dropdown
    Gui, TopBar:Add, DropDownList, x250 y3 w50 h200 vSelectedWindow gOnWindowSelect, win1||win2|win3|win4|win5|win6|win7|win8
    
    ; 4. Launch Nexus
    Gui, TopBar:Add, Button, x305 y3 w85 h24 gLaunchNexus, Launch Nexus
    
    ; 5. S/H Nexus
    Gui, TopBar:Add, Button, x395 y3 w75 h24 gToggleNexusGui, S/H Nexus
    
    ; 6. S/H Launcher
    Gui, TopBar:Add, Button, x475 y3 w85 h24 gToggleLauncherGui, S/H Launcher
    
    ; 7. Additional Scripts Dropdown
    Gui, TopBar:Add, DropDownList, x565 y3 w150 h200 vUtilityScriptDropdown, Accessoryawakening|altassistv2|citadelstore|expperhour|gear_enchanter|MemoryMonitor|MemoryMonitorMain|rupeesperhour||
    ...
    ; 11. Select All
    Gui, TopBar:Add, Button, x1020 y3 w75 h24 gToggleSelectAll, Select All
    
    ; 12. Win Checkboxes (Blue text)
    Gui, TopBar:Font, s8 cBlue, Segoe UI
    Gui, TopBar:Add, Checkbox, x1100 y6 w38 h18 vNetWin1, W1
    Gui, TopBar:Add, Checkbox, x1138 y6 w38 h18 vNetWin2, W2
    Gui, TopBar:Add, Checkbox, x1176 y6 w38 h18 vNetWin3, W3
    Gui, TopBar:Add, Checkbox, x1214 y6 w38 h18 vNetWin4, W4
    Gui, TopBar:Add, Checkbox, x1252 y6 w38 h18 vNetWin5, W5
    Gui, TopBar:Add, Checkbox, x1290 y6 w38 h18 vNetWin6, W6
...
CreateActivationBar() {
    ...
    Gui, ActBar:Add, Text, x130 y5 w120 h20, Activate Window
    
    ; Window activation buttons (initially hidden)
    Gui, ActBar:Font, s8 cWhite, Segoe UI
    Gui, ActBar:Add, Button, x245 y3 w60 h24 gActivateWin1 vActBtn1 Hidden, Win1
    Gui, ActBar:Add, Button, x310 y3 w60 h24 gActivateWin2 vActBtn2 Hidden, Win2
    Gui, ActBar:Add, Button, x375 y3 w60 h24 gActivateWin3 vActBtn3 Hidden, Win3
    Gui, ActBar:Add, Button, x440 y3 w60 h24 gActivateWin4 vActBtn4 Hidden, Win4
    Gui, ActBar:Add, Button, x505 y3 w60 h24 gActivateWin5 vActBtn5 Hidden, Win5
    Gui, ActBar:Add, Button, x570 y3 w60 h24 gActivateWin6 vActBtn6 Hidden, Win6
    Gui, ActBar:Add, Button, x635 y3 w60 h24 gActivateWin7 vActBtn7 Hidden, Win7
    Gui, ActBar:Add, Button, x700 y3 w60 h24 gActivateWin8 vActBtn8 Hidden, Win8
    
    ; Show Thumbnails button
    Gui, ActBar:Add, Button, x770 y3 w110 h24 gToggleThumbnails vBtnThumbnails, Show Thumbnails
    
    ; Reset Position button
    Gui, ActBar:Add, Button, x885 y3 w70 h24 gResetActBarPosition, Reset Pos
...
ToggleBarVisibility:
    ...
    GuiControlGet, btnText,, Button1
    If (InStr(btnText, "Hide")) {
        ; Hide entire bar and show only [+] button at top-left
        Gui, TopBar:Show, x0 y0 w30 h30
        GuiControl, TopBar:, Button1, [+]
        GuiControl, Move, Button1, x0 y0 w30 h30
    } Else {
        ; Show full bar
        global ScreenWidth
        Gui, TopBar:Show, x0 y0 w%ScreenWidth% h30
        GuiControl, TopBar:, Button1, Hide Bar
        GuiControl, Move, Button1, x5 y3 w60 h24
    }
return
...
LaunchNexus:
    Gui, TopBar:Submit, NoHide
    scriptPath := A_ScriptDir "\" SelectedWindow "\Rappelz Automation Nexus.ahk"
    ...
    ; Stop the auto-refresh timer temporarily
    SetTimer, AutoRefreshPIDs, Off
    
    Run, %scriptPath%
    ; Wait for script to fully launch (handles #SingleInstance Force restart)
    Sleep, 1500
    
    ; Debug: Show what we're looking for
    debugPath := A_ScriptDir "\" SelectedWindow
    
    ; Detect actual running PID after restart with retries
    attempts := 0
    thisPID := ""
    while (thisPID = "" && attempts < 5) {
        DetectRunningNexusScripts()
        thisPID := NexusPIDs[SelectedWindow]
        If (thisPID = "") {
            Sleep, 500
            attempts++
        }
    ...
    If (thisPID != "") {
        ; Position the Nexus window aligned with bottom of TopBar
        Sleep, 500  ; Give GUI time to fully render
        DetectHiddenWindows, On
        nexusWinID := WinExist("ahk_pid " thisPID)
        If (nexusWinID) {
            ; Position all Nexus windows at x1024 y30
            xPos := 1024
            yPos := 30
            
            WinMove, ahk_id %nexusWinID%, , %xPos%, %yPos%
        }
        DetectHiddenWindows, Off
        ; MsgBox, Launched %SelectedWindow% Nexus (PID: %thisPID%)
        
        ; Restart the timer now that we have the PID
        SetTimer, AutoRefreshPIDs, 500
    } Else {
        MsgBox, Launched %SelectedWindow% Nexus but could not detect PID.`nLooking for path: %debugPath%\Rappelz Automation Nexus.ahk
        ; Restart the timer even on failure
        SetTimer, AutoRefreshPIDs, 500
    }
return
...
ToggleNexusGui:
    ...
    ; First refresh PIDs to get latest running scripts
    DetectRunningNexusScripts()
    
    thisPID := NexusPIDs[SelectedWindow]
    
    If (!thisPID) {
        MsgBox, No Nexus script is running for %SelectedWindow%
        return
    }
    
    ; Find the GUI window for the selected Nexus script
    DetectHiddenWindows, On
    nexusWinID := WinExist("ahk_pid " thisPID)
    
    If (nexusWinID) {
        ; Check if selected window is visible
        isVisible := DllCall("IsWindowVisible", "Ptr", nexusWinID)
        If (isVisible) {
            ; Hide ALL Nexus clients
            Loop, 8 {
                winFolder := "win" A_Index
                checkPID := NexusPIDs[winFolder]
                If (checkPID) {
                    checkWinID := WinExist("ahk_pid " checkPID)
                    If (checkWinID) {
                        WinHide, ahk_id %checkWinID%
                    }
                }
            }
        } Else {
            ; Show only the SELECTED Nexus client
            WinShow, ahk_id %nexusWinID%
...
LaunchUtilityScript:
    ...
    If (UtilityScriptDropdown = "") {
        MsgBox, Please select a utility script to launch
        return
    }
    
    ; Check if script requires Tesseract OCR
    needsTesseract := (UtilityScriptDropdown = "Accessoryawakening" 
                    || UtilityScriptDropdown = "MemoryMonitor" 
                    || UtilityScriptDropdown = "expperhour" 
                    || UtilityScriptDropdown = "rupeesperhour")
    
    If (needsTesseract) {
        ; Check if Tesseract is installed
        RegRead, tesseractPath, HKLM\SOFTWARE\Tesseract-OCR, InstallDir
        If (ErrorLevel || !FileExist(tesseractPath "\tesseract.exe")) {
            MsgBox, 4, Tesseract OCR Required, Tesseract OCR is not installed.`n`nThis script requires Tesseract to function.`n`nWould you like to install it now?
            IfMsgBox Yes
            {
                installerPath := A_ScriptDir "\tesocr.exe"
                If FileExist(installerPath) {
                    Run, %installerPath%
                    MsgBox, Please complete the Tesseract installation, then launch the script again.
                } Else {
                    MsgBox, Installer not found: %installerPath%
                }
            }
            return
        }
    ...
    ; Check if it's altassistv2 (an .exe file)
    If (UtilityScriptDropdown = "altassistv2") {
        scriptPath := A_ScriptDir "\" UtilityScriptDropdown ".exe"
    } Else {
        scriptPath := A_ScriptDir "\" UtilityScriptDropdown ".ahk"
    }
    ...
    Run, %scriptPath%
    ; MsgBox, Launched %UtilityScriptDropdown%
return
