...
ShowMainGui() {
    ...
    Gui, Add, GroupBox, x10 y400 w460 h140, Actions
    ...
    Gui, Add, Button, x310 y450 w150 h25 gCheckForUpdates, Check for Updates (Script)
    ...
    Gui, Add, Button, x20 y480 w280 h25 gCheckGameUpdates, Check for Game Updates
    
    Gui, Show, w480 h555, Rappelz Multi-Client Launcher
}
...
; Launch clients using rappelz.bat
batPath := A_ScriptDir . "\rappelz.bat"
Loop, %selectedCount% {
    Run, "%batPath%"
    
    ; Wait for SplashLoading.exe to appear (timeout after 30 seconds)
    splashTimeout := A_TickCount + 30000
    Loop {
        Process, Exist, SplashLoading.exe
        if (ErrorLevel) {
            ; SplashLoading.exe found, break and continue to next client
            break
        }
        if (A_TickCount > splashTimeout) {
            MsgBox, 16, Error, Timeout waiting for SplashLoading.exe to appear for client %A_Index%
            ShowMainGui()
            Return
        }
        Sleep, 100
...
; Wait for all SplashLoading.exe processes to close before proceeding
allSplashTimeout := A_TickCount + 180000  ; 3 minute timeout
Loop {
    Process, Exist, SplashLoading.exe
    if (!ErrorLevel) {
        ; No SplashLoading.exe processes found, all done
        break
    }
    if (A_TickCount > allSplashTimeout) {
        MsgBox, 16, Error, Timeout waiting for all SplashLoading.exe processes to close
        ShowMainGui()
        Return
    }
    Sleep, 500
...
if (!FileExist(LauncherPath)) {
    MsgBox, 16, Error, Game launcher not found at:`n%LauncherPath%`n`nPlease set the correct launcher path.
    Return
}
Run, "%LauncherPath%"
Return
...
DownloadUpdate(NewVersion) {
    ...
    Progress, 10, Creating backup..., Updating Rappelz Nexus
    FileCreateDir, %A_ScriptDir%\backups
    FileCopy, %A_ScriptDir%\*.*, %A_ScriptDir%\backups\, 1
    
    Progress, 20, Downloading update from GitHub..., Updating Rappelz Nexus
    ...
    Progress, 50, Download complete. Extracting files..., Updating Rappelz Nexus
    FileRemoveDir, %ExtractPath%, 1
    FileCreateDir, %ExtractPath%
    
    Progress, 60, Extracting archive..., Updating Rappelz Nexus
    ...
    Progress, 70, Installing files..., Updating Rappelz Nexus
    ...
    Progress, 90, Cleaning up..., Updating Rappelz Nexus
    FileDelete, %ZipFile%
    FileRemoveDir, %ExtractPath%, 1
    ...
    Progress, 100, Update complete!, Updating Rappelz Nexus
    Sleep, 500
    Progress, Off
